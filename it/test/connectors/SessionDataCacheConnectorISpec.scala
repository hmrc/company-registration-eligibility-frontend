...
class SessionDataCacheConnectorISpec
  ...
  private val connector = app.injector.instanceOf[SessionDataCacheConnector]
  private val sessionCacheRepository = app.injector.instanceOf[SessionCacheRepository]
  ...
  "fetchUserAnswersFromSession" should {
    ...
    "return fully populated user answers when all values exist" in {
      ...
      await(sessionCacheRepository.putSession(secureRegister, true))
      await(sessionCacheRepository.putSession(paymentOption, false))
      await(sessionCacheRepository.putSession(identityVerification, true))
      ...
      result mustBe Some(
        UserAnswers(
          secureRegister = Some(true),
          paymentOption = Some(false),
          identityVerification = Some(true)
        )
      )
    }
